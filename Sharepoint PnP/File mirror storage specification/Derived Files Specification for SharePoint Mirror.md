...
└── <FolderPath>\
    ...
    │   │   ├── <UniqueIdPrefix>_v<PaddedVersion>_<FileName>.<ext>.pdf
    │   │   ├── <UniqueIdPrefix>_v<PaddedVersion>_<FileName>.<ext>_p001.jpg
    │   │   ├── <UniqueIdPrefix>_v<PaddedVersion>_<FileName>.<ext>_p002.jpg
    │   │   └── ...
    │   │
    │   └── <FileName>.<ext>.extracted\
    │       └── <UniqueIdPrefix>_v<PaddedVersion>_<FileName>.<ext>.json
    │
...
```
<UniqueIdPrefix>_v<PaddedVersion>_<FileName>.<ext>.<derived-ext>
```
...
|`ext`|Original source file extension|`xlsx`, `pptx`|
|`derived-ext`|Extension appropriate to derived type|`.pdf`, `.jpg`, `.json`|

### Page Bitmap Extension

For per-page JPEG renderings, append `_p<PaddedPage>` before the derived extension:

```
<UniqueIdPrefix>_v<PaddedVersion>_<FileName>.<ext>_p<PaddedPage>.jpg
```
...
|PDF rendering|`.rendered\`|`a1b2c3d4_v003.0_Budget.xlsx.pdf`|
|Page bitmap|`.rendered\`|`a1b2c3d4_v003.0_Budget.xlsx_p001.jpg`|
|Extracted content|`.extracted\`|`a1b2c3d4_v003.0_Budget.xlsx.json`|
...
└── 2025\Q1\
    ...
    │   │   ├── a1b2c3d4_v001.0_Budget.xlsx.pdf
    │   │   ├── a1b2c3d4_v001.0_Budget.xlsx_p001.jpg
    │   │   ├── a1b2c3d4_v002.0_Budget.xlsx.pdf
    │   │   ├── a1b2c3d4_v002.0_Budget.xlsx_p001.jpg
    │   │   ├── a1b2c3d4_v002.0_Budget.xlsx_p002.jpg
    │   │   ├── a1b2c3d4_v003.0_Budget.xlsx.pdf
    │   │   ├── a1b2c3d4_v003.0_Budget.xlsx_p001.jpg
    │   │   └── a1b2c3d4_v003.0_Budget.xlsx_p002.jpg
    │   │
    │   └── Budget.xlsx.extracted\
    │       ├── a1b2c3d4_v001.0_Budget.xlsx.json
    │       ├── a1b2c3d4_v002.0_Budget.xlsx.json
    │       └── a1b2c3d4_v003.0_Budget.xlsx.json
    │
    ├── Budget.xlsx
    └── Budget.xlsx.meta
```
...
1. **PDF**: Replace `.versions` with `.rendered`, append `.pdf` to the full version filename
2. **JPEG**: Replace `.versions` with `.rendered`, append `_p###.jpg` to the full version filename
3. **JSON**: Replace `.versions` with `.extracted`, append `.json` to the full version filename
...
$versionFileName = [System.IO.Path]::GetFileName($versionPath)            # a1b2c3d4_v003.0_Budget.xlsx
...
$pdfPath  = Join-Path $parentFolder "$sourceFileName.rendered\$versionFileName.pdf"
$jpgPath  = Join-Path $parentFolder "$sourceFileName.rendered\${versionFileName}_p001.jpg"  # per page
$jsonPath = Join-Path $parentFolder "$sourceFileName.extracted\$versionFileName.json"
```